AV.Home = class {
  constructor(game) {
    this.game = game;
    this.map = AV.buildHome(game.save.houseLevel);
    this.player = { x: this.map.spawnX, y: this.map.spawnY, r: 12, facing: 0, vx: 0, vy: 0 };
    this.prompt = "";
    this.near = null;
  }
  enter() {
    this.map = AV.buildHome(this.game.save.houseLevel);
    this.player.x = this.map.spawnX;
    this.player.y = this.map.spawnY;
  }
  update(dt) {
    const [ax, ay] = this.game.input.axis();
    this.player.vx = ax * 150;
    this.player.vy = ay * 150;
  ...
  draw(eng) {
    ...
    const c = eng.ctx;
    const [sx, sy] = eng.worldToScreen(this.player.x, this.player.y);
    c.fillStyle = "#e8d7c3";
    c.beginPath(); c.arc(sx, sy, this.player.r, 0, Math.PI * 2); c.fill();
    c.strokeStyle = "#c47a3a";
    c.beginPath();
    c.moveTo(sx, sy);
    c.lineTo(sx + Math.cos(this.player.facing) * 16, sy + Math.sin(this.player.facing) * 16);
    c.stroke();
    c.fillStyle = "#9a8773";
    c.font = "11px sans-serif";
    for (const [tx, ty, lab] of [[5,8,"STASH"],[8,8,"BENCH"],[5,5,"COT"],[16,10,"CINDERDEEP"]]) {
      const [lx, ly] = eng.worldToScreen(tx * AV.TILE + 6, ty * AV.TILE - 6);
